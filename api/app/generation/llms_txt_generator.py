...
async def generate_llms_txt_recommendation(
    ...
    raw_text = resp_llm.choices[0].message.content or "{}"
    input_tokens = resp_llm.usage.prompt_tokens if resp_llm.usage else 0
    output_tokens = resp_llm.usage.completion_tokens if resp_llm.usage else 0
