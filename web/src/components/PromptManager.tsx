...
const CAT_BADGE: Record<PromptCategory, string> = {
  awareness:      "bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300",
  evaluation:     "bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300",
  comparison:     "bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300",
  recommendation: "bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-300",
  brand:          "bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300",
};
...
const inputCls = "bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-indigo-500";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 text-sm">
        <span className="text-gray-500 dark:text-gray-400">Total: <strong className="text-gray-900 dark:text-white">{total}</strong></span>
        <span className="text-gray-500 dark:text-gray-400">Active: <strong className="text-green-600 dark:text-green-400">{activeCount}</strong></span>
        {CATEGORIES.map((c) => (
          <span key={c} className="text-gray-500 dark:text-gray-400">
            {c.charAt(0).toUpperCase() + c.slice(1)}: <strong className="text-gray-900 dark:text-white">{catCounts[c]}</strong>
          </span>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className={`${inputCls} w-56 placeholder-gray-400 dark:placeholder-gray-500`}
        />
        <select value={filterCat} onChange={(e) => { setFilterCat(e.target.value as PromptCategory | ""); setPage(1); }} className={inputCls}>
        ...
        <div className="ml-auto flex gap-2">
          ...
          <button onClick={() => setMode(mode === "csv" ? "none" : "csv")}
            className="px-4 py-2 text-sm font-semibold rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-white transition-colors">
            Upload CSV
          </button>
          <button onClick={() => setShowAudit((v) => !v)}
            className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors
              ${showAudit ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-white"}`}>
            Audit log
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-500 border-b border-gray-200 dark:border-gray-700 uppercase tracking-wider bg-gray-50 dark:bg-gray-800/50">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="px-5 py-8 text-center text-gray-400">No prompts found.</td></tr>
              ) : items.map((p) => {
                const isEditing = editId === p.id;
                return (
                  <tr key={p.id} className={`border-b border-gray-100 dark:border-gray-800 ${!p.is_active ? "opacity-50" : ""}`}>
                    <td className="px-5 py-3 max-w-md">
                      {isEditing ? (
                        <textarea rows={2} value={editText} onChange={(e) => setEditText(e.target.value)}
                          className="w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-indigo-500 resize-none" />
                      ) : (
                        <span className="text-gray-800 dark:text-gray-200" title={p.text}>
                    ...
                    <td className="px-5 py-3">
                      {isEditing ? (
                        <select value={editCat} onChange={(e) => setEditCat(e.target.value as PromptCategory)}
                          className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-indigo-500">
                    ...
                    <td className="px-5 py-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">{relDate(p.created_at)}</td>
                    <td className="px-5 py-3">
                      {isEditing ? (
                        <div className="flex gap-2">
                          <button onClick={saveEdit} className="text-xs text-indigo-500 hover:text-indigo-400">Save</button>
                          <button onClick={() => setEditId(null)} className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">Cancel</button>
                        </div>
                      ) : (
                        <button onClick={() => startEdit(p)} className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">Edit</button>
                      )}
                    </td>
                  </tr>
                );
        ...
        {totalPages > 1 && (
          <div className="px-5 py-3 flex items-center gap-4 text-sm text-gray-500 border-t border-gray-200 dark:border-gray-700">
