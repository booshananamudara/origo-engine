...
function PlatformResult({ item }: { item: PromptAnalysisItem }) {
  ...
  const displayText = truncated
    ? item.raw_response.slice(0, 300) + "…"
    : item.raw_response;

  return (
    <div className="grid grid-cols-2 gap-4 py-4 border-b border-gray-800/60 last:border-0">
      {/* Left: platform + raw response */}
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${PLATFORM_DOT[item.platform]}`} />
          <span className="text-xs font-semibold text-gray-300">
            {PLATFORM_LABEL[item.platform]}
          </span>
          <span className="text-xs text-gray-600">{item.model_used}</span>
          {item.latency_ms != null && (
            <span className="text-xs text-gray-600">{item.latency_ms}ms</span>
          )}
        </div>
      ...
      <div className="space-y-2 text-xs">
        {item.client_cited == null ? (
          <p className="text-gray-600 italic">Analysis pending…</p>
        ) : (
          <>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {item.client_prominence && item.client_prominence !== "not_cited" && (
                <span className="px-2 py-0.5 bg-gray-800 rounded-full text-gray-300 capitalize">
                  {item.client_prominence}
                </span>
              )}
              {item.citation_opportunity && (
                <span
                  className={`px-2 py-0.5 rounded-full font-semibold capitalize ${OPP_PILL[item.citation_opportunity] ?? ""}`}
                >
                ...
                <ul className="space-y-0.5">
                  ...
                  {item.content_gaps.length > 3 && (
                    <li className="text-gray-600">
                      +{item.content_gaps.length - 3} more
                    </li>
                  )}
                </ul>
              </div>
...
function PromptRow({ detail }: { detail: PromptDetail }) {
  ...
  return (
    <div className="border border-gray-800 rounded-xl overflow-hidden">
      ...
      >
        <span className="mt-0.5 text-gray-500 text-xs font-mono select-none">
          {expanded ? "▼" : "▶"}
        </span>
      ...
      {expanded && (
        <div className="border-t border-gray-800 bg-gray-900/50 px-6 py-2">
          <div className="grid grid-cols-2 gap-4 py-2 text-xs text-gray-500 border-b border-gray-800/60">
