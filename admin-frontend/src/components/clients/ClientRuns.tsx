        ...
        <div className="flex flex-col items-end gap-1">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white
              text-sm font-semibold disabled:bg-gray-700 disabled:text-gray-400 disabled:cursor-not-allowed
              transition-colors"
          >
              ...
              {data.items.map((run) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-semibold uppercase tracking-wide ${STATUS_STYLE[run.status] ?? ""}`}
                    >
                  ...
                  <td className="px-4 py-3">
                    {run.overall_citation_rate != null ? (
                      <span className={`font-mono text-sm font-semibold ${
                        run.overall_citation_rate >= 0.5 ? "text-green-400" :
                        run.overall_citation_rate >= 0.25 ? "text-amber-400" : "text-red-400"
                      }`}>
        ...
        {totalPages > 1 && (
          <div className="px-5 py-3 flex items-center justify-between border-t border-gray-800 text-sm text-gray-500">
            <button
              ...
              disabled={page === 1}
              className="disabled:opacity-40 hover:text-white transition-colors px-2 py-1 rounded hover:bg-gray-800"
            >
              ← Prev
            </button>
