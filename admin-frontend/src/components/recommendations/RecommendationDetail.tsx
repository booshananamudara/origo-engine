  ...
  return (
    <div className="space-y-4">
      {fields.map(({ label, key }) =>
        content[key] ? <FieldBlock key={key} label={label} value={String(content[key])} /> : null,
      )}
      {listFields.map(({ label, key }) => {
        const items = content[key] as string[] | undefined
        return items?.length ? <ListBlock key={key} label={label} items={items} /> : null
      })}
    </div>
  )
}
...
function SchemaView({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-5">
      {content.reasoning != null && <FieldBlock label="Reasoning" value={String(content.reasoning)} />}
      {schemas?.map((s, i) => (
        <div key={i} className="border-t pt-4 space-y-2">
          <Badge variant="outline" className="font-mono text-xs">{String(s.schema_type)}</Badge>
          ...
          {s.implementation_notes != null && (
            <p className="text-xs text-muted-foreground">{String(s.implementation_notes)}</p>
          )}
        </div>
      ))}
    </div>
    ...
    <div className="space-y-5">
      ...
      {sections?.length ? (
        <div className="space-y-2">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">New Sections</p>
          {sections.map((s, i) => (
            <div key={i} className="bg-muted rounded-md p-3 space-y-1">
              <p className="text-sm font-semibold">{String(s.section_title)}</p>
              <pre className="text-xs whitespace-pre-wrap font-mono">{String(s.content)}</pre>
      ...
      {mods?.length ? (
        <div className="space-y-2">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Modifications</p>
          {mods.map((m, i) => (
            <div key={i} className="bg-muted rounded-md p-3 space-y-1">
              <p className="text-xs text-muted-foreground">Section: {String(m.existing_section)}</p>
...
export function RecommendationDetailPage() {
  ...
  const approveMut = useMutation({
    ...
    onSuccess: () => { invalidate(); setActiveAction(null); toast.success("Recommendation approved") },
    onError: () => toast.error("Failed to approve"),
  })
  const rejectMut = useMutation({
    mutationFn: (n: string) => recommendationsApi.reject(id!, n),
    onSuccess: () => { invalidate(); setActiveAction(null); toast.success("Recommendation rejected") },
    onError: () => toast.error("Failed to reject"),
  })
  const revisionMut = useMutation({
    mutationFn: (n: string) => recommendationsApi.requestRevision(id!, n),
    onSuccess: () => { invalidate(); setActiveAction(null); toast.success("Revision requested") },
    onError: () => toast.error("Failed to request revision"),
  })
  const implementMut = useMutation({
    mutationFn: (n: string) => recommendationsApi.implement(id!, n || undefined),
    onSuccess: () => { invalidate(); setActiveAction(null); toast.success("Marked as implemented") },
    onError: () => toast.error("Failed to update"),
  })

  const isMutating = approveMut.isPending || rejectMut.isPending || revisionMut.isPending || implementMut.isPending
  ...
  return (
    <BlurFade>
      <PageHeader
        breadcrumbs={[
          { label: "Recommendations", href: `/recommendations?client_id=${clientId}` },
            ...
            <div className="lg:col-span-2 space-y-4">
                  ...
                  <CardContent className="flex flex-col gap-2">
                    {canApprove && (
                      <Button
                        ...
                        onClick={() => { setNotes(""); setActiveAction("approve") }}
                      >
                        Approve
                      </Button>
                    )}
                    {canRequestRevision && (
                      <Button
                        variant="outline"
                        className="w-full"
                        onClick={() => { setNotes(""); setActiveAction("request_revision") }}
                      >
                        Request Revision
                      </Button>
                    )}
                    {canReject && (
                      <Button
                        variant="destructive"
                        className="w-full"
                        onClick={() => { setNotes(""); setActiveAction("reject") }}
                      >
                        Reject
                      </Button>
                    )}
                    {canImplement && (
                      <Button
                        className="w-full"
                        onClick={() => { setNotes(""); setActiveAction("implement") }}
                      >
                        Mark Implemented
                      </Button>
                    )}
              ...
              {rec.target_query && (
                <Card>
                  ...
                  <CardContent>
                    <p className="text-sm italic text-muted-foreground">"{rec.target_query}"</p>
                  </CardContent>
                </Card>
              )}
              ...
              {rec.analysis_data && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Why This Was Generated</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Cited</p>
                        <p className={`text-sm font-semibold ${rec.analysis_data.client_cited ? "text-emerald-600" : "text-red-600"}`}>
                          {rec.analysis_data.client_cited ? "Yes" : "No"}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Prominence</p>
                        <p className="text-sm capitalize">{rec.analysis_data.client_prominence}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Sentiment</p>
                        <p className="text-sm capitalize">{rec.analysis_data.client_sentiment}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Opportunity</p>
                        <p className={`text-sm font-semibold capitalize ${
                          rec.analysis_data.citation_opportunity === "high" ? "text-red-600" :
                          rec.analysis_data.citation_opportunity === "medium" ? "text-amber-600" :
                          "text-blue-600"
                        }`}>
                    ...
                    {rec.analysis_data.content_gaps.length > 0 && (
                      <div>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">Content Gaps</p>
                    ...
                    {rec.analysis_data.reasoning && (
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{rec.analysis_data.reasoning}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
                  ...
                  <div key={h.id} className="flex gap-3 items-start">
                    ...
                    <div>
                      <p className="text-sm">
                        ...
                        {h.old_status ? (
                          <> changed status from <span className="text-muted-foreground">{h.old_status}</span> to <span className="font-medium">{h.new_status}</span></>
                        ) : (
                          <> created with status <span className="font-medium">{h.new_status}</span></>
                        )}
                      </p>
                      {h.notes && <p className="text-xs text-muted-foreground mt-0.5">"{h.notes}"</p>}
                      <p className="text-xs text-muted-foreground mt-0.5">{fmtDate(h.created_at)}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
